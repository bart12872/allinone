...
class Sea_Db_Table extends Zend_Db_Table {
	...
	protected function _fetch($select) {
		...
		foreach ($this->info(Zend_Db_Table::METADATA) as $field => $data) {
			switch (preg_replace('/\(.*/', '', $data['DATA_TYPE'])) {
				...
				case 'datetime' : $select->columns(array('format_' . $field => 'DATE_FORMAT(' . $field . ',  "%d/%m/%Y")'));break;
			}
		}
	 ...
	 * @param Array $row
	 */
	protected function findFull($primary) {
	    
	    // on rechrehce le premier enregistrement
		if ($result = $this->find($primary)) {
