...
class Sea_Form_Decorator_SeaErrors extends Zend_Form_Decorator_Abstract{
	...
	public function render($content) {
		...
		if ($e->hasErrors()) {
	        ...
	        $decorator->setOptions(array('tag' => 'div','class' => 'form-error'));			
	        $content = $decorator->render(sprintf('<p>%s</p>', implode('<br/>', $e->getMessages())));
			
			// on ajourte la class erreur a l'element
			$e->setAttrib('class', sprintf('%s %s', $e->getAttrib('class'), 'ui-state-error'));
		}
