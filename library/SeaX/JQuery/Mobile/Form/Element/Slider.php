 ...
 */
class SeaX_JQuery_Mobile_Form_Element_Slider extends Zend_Form_Element_Multi
{
    /**
     * Use formSelect view helper by default
     * @var string
     */
    public $helper = 'mobileFormSlider';
     ...
     */
    public function __construct($spec, $label = '' , $min, $max, $step, $required = false ) {
    	
    	// construction du parent
    	parent::__construct($spec);
    	
    	// on ajoute les options
    	$this->setMultiOptions(array_fill_all(range($min,$max)));
    	
    	//on ajoute le min, max et le step
    	$this->addMultiOptions(array('min' => $min, 'max' => $max, 'step' => $step));
    	
    	//on ajoute les validators 
    	$this->addValidator(new Zend_Validate_Between($min,$max), true);
