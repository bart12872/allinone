 ...
 *
 */
class Default_Datagrid_BinsearchMatch extends Application_Model_Datatable {
	...
	public function init() {
		...
		// CREATION DES DATA
		$source = $db->select()	->from(['c' => 'binsearch_collection'], 'title')
								->join(['g' => 'binsearch_group'], 'g.binsearch_group_id=c.binsearch_group_id', 'short_title')
								->join(['m' => 'binsearch_match'], 'c.binsearch_collection_id=m.binsearch_collection_id')
		...
		$this->addText('Collection', 'title')->width('550px')->align('left')->sort('title');
		$this	->addText('Groupe', 'short_title')
				->setStrainerSelect(array('-- Choisissez --') + $db->fetchPairs($select))
				->sort('short_title')
				->align('center');
		
		$this->addText('Nom', 'name')->sort('name');
		$this->addText('Sai.', 'season')->sort('season');
		$this->addText('Epi.', 'episode')->sort('episode');
